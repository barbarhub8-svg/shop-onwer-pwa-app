  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-slate-600">Shop Owner Portal</p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-sm sm:rounded-2xl sm:px-10 border border-slate-200">
          <form className="space-y-5" onSubmit={handleLogin}>
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input
                  type="email"
                  defaultValue="owner@royalglow.com"
          ...
          <div className="mt-6">
            ...
            <div className="mt-6">
              <button
                onClick={() => showToast('Google login simulated')}
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 border border-slate-300 rounded-xl shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 transition-colors"
              >
            ...
            <div className="mt-6 text-center">
              <button onClick={() => showToast('Redirect to Registration Form')} className="text-sm font-medium text-blue-600 hover:text-blue-500">
      ...
      <Modal 
        ...
        footer={
          <>
            <button onClick={() => setIsForgotModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
            <button onClick={() => { setIsForgotModalOpen(false); showToast('Reset link sent!'); }} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Send Link</button>
          </>
        }
      >
        <p className="text-sm text-slate-600 mb-4">Enter your registered email address and we'll send you a link to reset your password.</p>
        <input type="email" placeholder="Email address" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none" />
      </Modal>
    </div>
  );
}