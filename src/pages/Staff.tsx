...
export default function Staff() {
  ...
  const handleSaveStaff = async (e: React.FormEvent) => {
    ...
    try {
      let finalAvatarUrl = editingStaff ? editingStaff.avatar_url : 'https://i.pravatar.cc/150';
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="text-center py-10 text-slate-500">Loading staff...</div>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {staffList.map(staff => (
            <div key={staff.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative group">
              ...
              <div className="flex items-center gap-4">
                <img src={staff.avatar_url || 'https://i.pravatar.cc/150'} alt={staff.name} className="w-16 h-16 rounded-full object-cover border border-slate-200" />
                <div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                <div>
                  ...
                  <p className="text-lg font-bold text-slate-900 flex items-center gap-2 mt-1">
                    <Calendar className="w-4 h-4 text-blue-600" /> 0
                  </p>
                </div>
                <div>
          ...
          {staffList.length === 0 && (
            <div className="col-span-full text-center py-10 text-slate-500">No staff found. Add team members to get started.</div>
          )}
        </div>
      )}
        ...
        {viewingStaff && (
          <div className="space-y-6">
            <div className="flex flex-col items-center text-center">
              <img src={viewingStaff.avatar_url || 'https://i.pravatar.cc/150'} alt={viewingStaff.name} className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-md mb-4" />
